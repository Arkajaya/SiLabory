<x-app-layout>
    <div class="py-6 h-dvh">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="flex justify-between items-center mb-4">
                <h1 class="text-4xl font-semibold text-[#473472] tracking-wider underline underline-offset-4">Items List</h1>
                <x-primary-button
                    x-data
                    x-on:click="$dispatch('open-modal', 'create-item')" class="flex items-center justify-center bg-[#53629E]  px-2 ">
                                    ...
                                    <tr class="bg-neutral-primary border-b border-default">
                                        ...
                                        <td class="px-6 py-4">
                                            <div class="w-20 h-20 mx-auto">
                                                <img src="{{ asset('storage/photos/' . $item->photo) }}" alt="{{ $item->name }}" class="w-20 h-20 object-cover rounded-md">
                                            </div>
                                        </td>
                                        <td class="px-6 py-4">
                                            ...
                                            <x-primary-button
                                                x-data
                                            x-on:click="$dispatch('open-modal', 'edit-item-{{ $item->id }}')" class="bg-none text-orange-500"
                                            >
                                                Edit
                                            </x-primary-button>
    ...
    <x-modal name="create-item" :show="false" class="rounded-2xl">
            ...
            <div class="px-6 pb-6">
                <form method="POST" action="{{ route('items.store') }}" class="mt-6 space-y-6" enctype="multipart/form-data">
                    @csrf
                ...
                <form method="POST" action="{{ route('items.update', $item->id) }}" class="mt-6 space-y-6">
                    ...
                    <div class="p-6">
                        <form method="POST" action="{{ route('items.update', $item->id) }}" class="mt-6 space-y-6" enctype="multipart/form-data">
                            ...
                            <div>
                                <x-input-label class="mb-2" value="Category" id="category_id" />
                                <select name="category_id" value="{{ old('category_id', $item->category_id) }}" class="w-full border rounded-md text-slate-500" required>
                                    <option value=""  disabled selected>Select Category</option>
                                        @foreach ($categories as $category)
                                        <option value="{{ $category->id }}">{{ $category->name }}</option>
                                        @endforeach
                            ...
                            <div>
                                <x-input-label class="mb-2" value="Photo" id="photo" value="{{ old('photo', $item->photo) }}"/>
                                <input type="file" name="photo" accept="image/*" class="w-full" />
                            </div>
